...
export default function Chat() {
  ...
  useEffect(() => {
    const pusher = new Pusher(process.env.NEXT_PUBLIC_PUSHERKEY, { 
      ...
      channel.bind("message", data => {
        setMessageLog(prev => [...prev, {message: data.message}])
        console.log(data)
      })
  ...
  function openChatClick() {
    setIsChatOpen(true)
    setUnreadCount(0)
    setTimeout(() => chatField.current.focus(), 350)
  }
       ...
       {unreadCount > 0 && <span className="chat-unread-badge">{unreadCount}</span>}
          ...
          <div className={ isChatOpen ? "chat-container chat-container--visible" : "chat-container" }>
            <div className="chat-title-bar">
              ...
              <svg onClick={closeChatClick} xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" className="bi bi-x-square-fill" viewBox="0 0 16 16" >
            ...
            <div ref={chatLogElement} className="chat-log">
                ...
                {messageLog.map((item, index)=> {
                  return (
                    <div className={item.selfMessage ? "chat-message chat-message--self" : "chat-message"} key={index}>
